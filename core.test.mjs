...
const sequence = createChallengeSequence(['E12', 'E12', 'E24', 'E24'], () => 0);
assert.deepEqual(sequence.map(item => item.tolerance), [10, 10, 5, 5]);
assert.equal(new Set(sequence.map(item => item.nominal)).size, 4);
